...
df_wide = pd.read_csv(file_path,skiprows=1)

# Fix column typo if needed
df_wide = df_wide.rename(columns={"lables.3": "labels.3"})
...
# Generate NTU range labels like "1-2", "2-3", ...
...
# -------- 9. Compute F1 Score per NTU Category --------
f1_by_ntu = (
    df_long.groupby('NTU Category')
    .apply(lambda g: f1_score(g['labels'], g['predictions'], zero_division=0))
    .reset_index(name='F1 Score')
)

# -------- 10. Plot Bar Chart --------
plt.figure(figsize=(10, 6))
plt.bar(f1_by_ntu['NTU Category'], f1_by_ntu['F1 Score'], color='skyblue', edgecolor='black')
plt.xlabel("NTU Category")
plt.ylabel("F1 Score")
plt.title("F1 Score vs NTU Category")
