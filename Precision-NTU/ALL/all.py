...
# -------- 9. Compute Precision per NTU Category --------
precision_by_ntu = (
    df_long.groupby('NTU Category')
    .apply(lambda g: precision_score(g['labels'], g['predictions'], zero_division=0))
    .reset_index(name='Precision')
)

# -------- 10. Plot Bar Chart --------
plt.figure(figsize=(10, 6))
plt.bar(precision_by_ntu['NTU Category'], precision_by_ntu['Precision'], color='lightgreen', edgecolor='black')
plt.xlabel("NTU Category")
plt.ylabel("Precision")
plt.title("Precision vs NTU Category")
